...
function createBillboard(scene, highlightLayer, interval, router) {
    ...
    let pillar, bottomFrame, billboard;

    const xl = window.matchMedia("(min-width: 1280px)");
    const lg = window.matchMedia("(min-width: 1024px)");

    if (xl.matches) {
        pillar = CylinderBuilder.CreateCylinder("piller", {diameter: 4, height: 18}, scene);
        pillar.position = new Vector3(-24, -12, 4);
        bottomFrame = CylinderBuilder.CreateCylinder("bottomFrame", {diameter: 1.2, height: 71.8}, scene);
        bottomFrame.position = new Vector3(-24, -5, 4);
        billboard = PlaneBuilder.CreatePlane("billboard", {width: 72, height: 38, sideOrientation: Mesh.DOUBLESIDE}, scene);
        billboard.position = new Vector3(-24, 14, 4);
    } else if (lg.matches) {
        pillar = CylinderBuilder.CreateCylinder("piller", {diameter: 2, height: 24}, scene);
        pillar.position = new Vector3(-18, -12, 4);
        bottomFrame = CylinderBuilder.CreateCylinder("bottomframe", {diameter: 1.2, height: 54}, scene);
        bottomFrame.position = new Vector3(-18, 0, 4);
        billboard = PlaneBuilder.CreatePlane("billboard", {width: 54, height: 28, sideorientation: Mesh.doubleside}, scene);
        billboard.position = new Vector3(-18, 14, 4);
    } else {
        pillar = CylinderBuilder.CreateCylinder("piller", {diameter: 2, height: 24}, scene);
        pillar.position = new Vector3(-4, -12, 4);
        bottomFrame = CylinderBuilder.CreateCylinder("bottomFrame", {diameter: 1.2, height: 28}, scene);
        bottomFrame.position = new Vector3(-4, 0, 4);
        billboard = PlaneBuilder.CreatePlane("billboard", {width: 28, height: 16, sideOrientation: Mesh.DOUBLESIDE}, scene);
        billboard.position = new Vector3(-4, 8, 4);
    }
...
function createBuoy(scene, highlightLayer, water, router) {
    let rightBuoy, middleBuoy, leftBuoy;

    const xl = window.matchMedia("(min-width: 1280px)");
    const lg = window.matchMedia("(min-width: 1024px)");
    if (xl.matches) {
        rightBuoy = SphereBuilder.CreateSphere("rightBuoy", {diameter: 6}, scene);
        rightBuoy.position = new Vector3(22, -13, 20);
        rightBuoy.rotation = new Vector3(0, 2.1 * -Math.PI / 2, 0);
        middleBuoy = rightBuoy.clone("middleBuoy");
        middleBuoy.position = new Vector3(-6.5, 0, 0);
        leftBuoy = middleBuoy.clone("leftBuoy");
        leftBuoy.position = new Vector3(-13, 0, 0);
    } else if (lg.matches) {
        rightBuoy = SphereBuilder.CreateSphere("rightBuoy", {diameter: 6}, scene);
        rightBuoy.position = new Vector3(20, -13, -10);
        rightBuoy.rotation = new Vector3(0, 2.1 * -Math.PI / 2, 0);
        middleBuoy = rightBuoy.clone("middleBuoy");
        middleBuoy.position = new Vector3(-6.5, 0, 0);
        leftBuoy = middleBuoy.clone("leftBuoy");
        ...
        rightBuoy = SphereBuilder.CreateSphere("rightBuoy", {diameter: 6}, scene);
        rightBuoy.position = new Vector3(0, -13, -10);
        rightBuoy.rotation = new Vector3(0, 2.1 * -Math.PI / 2, 0);
        middleBuoy = rightBuoy.clone("middleBuoy");
        middleBuoy.position = new Vector3(-6.5, 0, 0);
        leftBuoy = middleBuoy.clone("leftBuoy");
...
const onSceneReady = (scene, interval, router) => {
    ...
    const camera = new ArcRotateCamera("Camera", 3 * Math.PI / 1.78, Math.PI / 1.9, 80, Vector3.Zero(), scene);
...
function Landing3D() {
    ...
    useEffect(() => {
        ...
        if (scene.isReady()) {
            ...
            scene.onReadyObservable.addOnce((scene) => onSceneReady(scene, interval, router));
        }

...
function createBillboard(scene, highlightLayer, router) {
    const mat = createBgMaterial("static/assets3d/textures/vgreen.jpeg", scene);
    mat.shadowLevel = 0.4;
    ...
    const bottomFrame = CylinderBuilder.CreateCylinder("bottomFrame", {diameter: 1, height: 51.8}, scene);
    bottomFrame.position = new Vector3(-24, -5, 4);
    bottomFrame.rotation = new Vector3(0, - Math.PI / 5.4, Math.PI / 2);
    bottomFrame.material = mat;

    const billboard = PlaneBuilder.CreatePlane("billboard", {width: 52, height: 30, sideOrientation: Mesh.DOUBLESIDE}, scene);
    billboard.position = new Vector3(-24, 10, 4);
...
function* loadMeshesIntoFrames(scene, highlightLayer, router, removeOverlay) {
    const skybox = createSkyBox(scene);
    yield;
    createLargeGround(scene);
    yield;
    const billboard = createBillboard(scene, highlightLayer, router);
    yield;
...
const onSceneReady = (scene, router) => {
    const canvas = scene.getEngine().getRenderingCanvas();

    const camera = new ArcRotateCamera("Camera", 3 * Math.PI / 1.9, Math.PI / 1.9, 80, Vector3.Zero(), scene);
    ...
    scene.onBeforeRenderObservable.runCoroutineAsync(loadMeshesIntoFrames(scene, highlightLayer, router, () => {
...
function Landing3D() {
    ...
    return (
        <div className="flex justify-center items-center w-[100vw] h-[100vh] relative">
            <SceneComponent antialias onSceneReady={(scene) => onSceneReady(scene, router)} className="outline-none w-full h-full" />
        </div>
    );
}
